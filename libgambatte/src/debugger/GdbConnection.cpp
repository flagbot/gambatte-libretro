...
GdbConnection::GdbConnection(int sockfd) : sockfd(sockfd)
{

}

void GdbConnection::ReadInput()
{
    std::tuple<uint8_t*, size_t> target = in_buffer.Reserve(RECV_BUF_SIZE);
    size_t r = read(sockfd, (char*) std::get<0>(target), std::get<1>(target));
    if(r <= 0) {
        SignalError();
    } else {
        LOG(DEBUG) << "gdb connection got " << r << " bytes in";
...
util::Buffer *GdbConnection::Process(bool &interrupted) {
	...
	while(in_buffer.Read(ch)) {
		...
		case State::WAITING_PACKET_OPEN:
			...
			if(ch != '$') {
				LOG(ERROR) << "packet opened with bad character " << ch;
				SignalError();
				return nullptr;
			}
		...
		case State::CHECKSUM_1:
			...
			if(DecodeHexByte(checksum_hex) != checksum) {
				if(ack_enabled) {
					char msg = '-';
					write(sockfd, &msg, sizeof(msg)); // request retransmission
				} else {
				...
				if(ack_enabled) {
					char msg = '+';
					write(sockfd, &msg, sizeof(msg)); // ok
				}
...
void GdbConnection::SendMessage(util::Buffer &buffer, char packet_ident)
{
    std::unique_lock<std::mutex> lock(mutex);
    out_buffer.Write(packet_ident);
    uint8_t checksum = 0;
    char ch;
    ...
    while(out_buffer.ReadAvailable()) {
        ssize_t r = write(sockfd, (char*) out_buffer.Read(), out_buffer.ReadAvailable());
        if(r < 0) {
            SignalError();
            return;
        }
...
void GdbConnection::Notification(util::Buffer &buffer)
{
    sleep(1);
    LOG(DEBUG) << "Sending message: " << buffer.GetString();
    SendMessage(buffer, '%');
}
...
void GdbConnection::SignalError() {
	LOG(ERROR) << "gdb connection error";
...
void GdbConnection::Decode(std::vector<uint8_t> &out, util::Buffer &packet) {
	...
	while(packet.ReadAvailable()) {
		...
		packet.MarkRead(1); // consume
		if(!packet.ReadAvailable()) {
			LOG(ERROR) << "unexpectedly odd number of nybbles";
			return;
		}
...
void GdbConnection::Encode(uint64_t n, size_t size, util::Buffer &out_buffer, bool little_endian) {
    int high = size == 0 ? 64 : size * 8;
    int low = 0;
    int mult = little_endian ? -1 : 1;
	for(int bit_offset = little_endian ? high : low + 8; little_endian ? (bit_offset > low) : (bit_offset <= high); bit_offset += 8*mult) {
